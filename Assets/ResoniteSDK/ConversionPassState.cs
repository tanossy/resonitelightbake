...
public static class ConversionPassState
{
    ...
    // Moved here from ResoniteLinkWindow (was a plain field on the main SDK panel, read via
    // SceneConverter's `_window.ForceRefreshGeneratedLightmaps`) so the toggle can live in the
    // Lightmap Pipeline panel instead, keeping the main panel limited to what's actually part of
    // vanilla Resonite.UnitySDK. Same decoupling pattern already used by ToneMapCompensationState.
